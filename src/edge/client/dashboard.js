...
(function () {
  ...
  var formatCell = function (value, col) {
    ...
    switch (fmt) {
      ...
      case "method":
        return (
          '<span class="' + methodClass(value) + '">' + esc(value) + "</span>"
        );
      case "json": {
        ...
        var preview =
          typeof value === "object"
            ? compactPreview(value, 100)
            : String(value);
        return (
          '<span class="ss-dash-data-preview" style="cursor:default">' +
          esc(preview) +
          "</span>"
        );
      }
  ...
  var sidebarCollapsed =
    localStorage.getItem("ss-dash-sidebar") === "collapsed";
  ...
  var applySidebar = function () {
    if (sidebar)
      sidebar.classList.toggle("ss-dash-collapsed", sidebarCollapsed);
    if (sidebarToggle)
  ...
  var switchSection = function (name) {
    ...
    navItems.forEach(function (item) {
      item.classList.toggle(
        "ss-dash-active",
        item.getAttribute("data-ss-section") === name,
      );
    });
  ...
  var fetchOverview = function () {
    Promise.all([
      ...
      .catch(function () {
        ...
        if (el)
          el.innerHTML =
            '<div class="ss-dash-empty">Failed to load overview</div>';
      });
  };
  ...
  var renderOverview = function (data, chartData) {
    ...
    if (data.queryStats && data.queryStats.total !== undefined)
      lastQueryStats = data.queryStats;
    if (data.recentErrors) lastRecentErrors = data.recentErrors;
    if (data.topEvents && data.topEvents.length > 0)
      lastTopEvents = data.topEvents;
    if (
      data.emailActivity &&
      (data.emailActivity.sent ||
        data.emailActivity.queued ||
        data.emailActivity.failed)
    )
    ...
    if (data.statusDistribution)
      lastStatusDistribution = data.statusDistribution;
    ...
    if (slowest.length > 0) {
      ...
      html +=
        '<div class="ss-dash-empty" style="min-height:60px">No data yet</div>';
    }
    html += "</div>";
    ...
    if (recentErrors.length > 0) {
      ...
      html +=
        '<div class="ss-dash-empty" style="min-height:60px">No recent errors</div>';
    }
    html += "</div>";
    ...
    if (topEvents.length > 0) {
      ...
      topEvents.slice(0, 5).forEach(function (ev) {
        html +=
          ...
          encodeURIComponent(
            ev.name || ev.eventName || ev.event_name || ev.event || "",
          ) +
          '" class="ss-dash-widget-row-link"><span title="' +
      ...
      html +=
        '<div class="ss-dash-empty" style="min-height:60px">No events yet</div>';
    }
    html += "</div>";
    ...
    if (cacheStats) {
      ...
      html +=
        '<div class="ss-dash-empty" style="min-height:60px">Not available</div>';
    }
    html += "</div>";
    ...
    if (slowQueries.length > 0) {
      ...
      html +=
        '<div class="ss-dash-empty" style="min-height:60px">No queries yet</div>';
    }
    html += "</div>";
  ...
  var renderSparklineSVG = function (points) {
    ...
    var coords = points.map(function (v, i) {
      return (
        (i * step).toFixed(1) +
        "," +
        (h - ((v / max) * h * 0.9 + h * 0.05)).toFixed(1)
      );
    });
  ...
  var renderBarChart = function (data) {
    ...
    var svg =
      '<svg viewBox="0 0 ' + w + " " + h + '" class="ss-dash-chart-svg">';
    ...
    // Tooltip element
    svg +=
      '<div class="ss-dash-chart-tooltip" id="ss-dash-chart-tip" style="display:none"></div>';
    ...
    if (svgEl && tip) {
      ...
      var showTip = function (idx, x) {
        ...
        tip.innerHTML =
          ...
          " requests</div>" +
          (errors > 0
            ? '<div style="color:var(--ss-red-fg)">' + errors + " errors</div>"
            : "");
      ...
      var hideTip = function () {
        ...
        dots.forEach(function (dot) {
          dot.setAttribute(
            "r",
            dot.classList.contains("ss-dash-chart-dot-err") ? "2" : "2.5",
          );
          dot.style.opacity = "1";
        });
      };
  ...
  var fetchRequests = function () {
    ...
    if (requestUrlFilter)
      suffix += "&url=" + encodeURIComponent(requestUrlFilter);
    if (requestStatusFilter)
      suffix += "&status=" + encodeURIComponent(requestStatusFilter);
    requestUrlFilter = "";
    requestStatusFilter = "";
  ...
  var renderRequests = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-requests-body",
        '<div class="ss-dash-empty">No requests recorded yet</div>',
      );
  ...
  var renderQueries = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-queries-body",
        '<div class="ss-dash-empty">No queries recorded yet</div>',
      );
    ...
    items.forEach(function (q) {
      ...
      html +=
        ...
        ((sqlMethod || "").toLowerCase() === "select"
          ? '<button class="ss-dash-explain-btn" data-query-id="' +
            q.id +
            '">EXPLAIN</button>'
          : "") +
        "</td>" +
  ...
  var renderPlanNode = function (node, depth) {
    ...
    var indent = depth * 20;
    var html =
      '<div class="ss-dash-explain-node" style="margin-left:' + indent + 'px">';
    ...
    var alias =
      node["Alias"] && node["Alias"] !== node["Relation Name"]
        ? " (" + esc(node["Alias"]) + ")"
        : "";
    var idx = node["Index Name"]
      ? " using <em>" + esc(node["Index Name"]) + "</em>"
      : "";
    ...
    if (metrics.length > 0) {
      html +=
        '<div class="ss-dash-explain-metrics">' +
        metrics.join(" &middot; ") +
        "</div>";
    }
  ...
  var renderExplainPlan = function (plan) {
    ...
    if (topPlan && topPlan["Plan"]) {
      return (
        '<div class="ss-dash-explain-result">' +
        renderPlanNode(topPlan["Plan"], 0) +
        "</div>"
      );
    }
      ...
      btn.addEventListener("click", function (e) {
        ...
        fetchJSON(API + "/queries/" + id + "/explain")
          .then(function (data) {
            ...
            if (prev && prev.classList.contains("ss-dash-explain-row"))
              prev.remove();
  ...
  var fetchEvents = function () {
    ...
    if (eventNameFilter)
      url += "&event_name=" + encodeURIComponent(eventNameFilter);
    eventNameFilter = "";
    fetchJSON(url)
      .then(function (data) {
        renderEvents(data);
      ...
      .catch(function () {
        setInner(
          "ss-dash-events-body",
          '<div class="ss-dash-empty">Failed to load events</div>',
        );
      });
  };
  ...
  var renderEvents = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-events-body",
        '<div class="ss-dash-empty">No events recorded yet</div>',
      );
  ...
  var fetchRoutes = function () {
    fetchJSON(API + "/routes")
      ...
      .catch(function () {
        setInner(
          "ss-dash-routes-body",
          '<div class="ss-dash-empty">Failed to load routes</div>',
        );
      });
  };
  ...
  var renderRoutes = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-routes-body",
        '<div class="ss-dash-empty">No routes available</div>',
      );
      return;
    }
    ...
    items.forEach(function (r) {
      html +=
        ...
        TRUNC +
        '" title="' +
        (r.middleware && r.middleware.length
          ? esc(r.middleware.join(", "))
          : "-") +
        '">' +
        (r.middleware && r.middleware.length
          ? esc(r.middleware.join(", "))
  ...
  var fetchLogs = function () {
    ...
    if (logReqIdFilter)
      params += "&request_id=" + encodeURIComponent(logReqIdFilter);
    ...
    fetchJSON(API + "/logs?" + params)
      ...
      .catch(function () {
        setInner(
          "ss-dash-logs-body",
          '<div class="ss-dash-empty">Failed to load logs</div>',
        );
      });
  };
  ...
  var renderLogs = function (data) {
    ...
    if (items.length === 0) {
      ...
      setInner(
        "ss-dash-logs-body",
        '<div class="ss-dash-empty">No log entries' + hint + "</div>",
      );
  ...
  if (logReqIdInput) {
    logReqIdInput.addEventListener("input", function () {
      ...
      if (logReqIdClear)
        logReqIdClear.style.display = logReqIdFilter ? "" : "none";
      getPage("logs").page = 1;
      fetchLogs();
    });
  }
  ...
  var renderFilterChips = function () {
    ...
    container
      .querySelectorAll(".ss-dash-filter-chip-remove")
  ...
  var fetchEmails = function () {
    ...
    if (emailStatusFilter)
      url += "&status=" + encodeURIComponent(emailStatusFilter);
    emailStatusFilter = "";
    fetchJSON(url)
      .then(function (data) {
        renderEmails(data);
      ...
      .catch(function () {
        setInner(
          "ss-dash-emails-body",
          '<div class="ss-dash-empty">Failed to load emails</div>',
        );
      });
  };
  ...
  var renderEmails = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-emails-body",
        '<div class="ss-dash-empty">No emails captured yet</div>',
      );
  ...
  var emailPreviewClose = document.getElementById(
  ...
  var fetchTraces = function () {
    ...
    fetchJSON(API + "/traces?page=" + ps.page + "&limit=" + PER_PAGE)
      ...
      .catch(function () {
        setInner(
          "ss-dash-timeline-body",
          '<div class="ss-dash-empty">Failed to load traces</div>',
        );
      });
  };
  ...
  var renderTraces = function (data) {
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-timeline-body",
        '<div class="ss-dash-empty">No requests traced yet</div>',
      );
  ...
  var renderWaterfall = function (container, trace) {
    ...
    var total =
      trace.total_duration || trace.totalDuration || trace.duration || 1;
    ...
    if (spans.length === 0) {
      html +=
        '<div class="ss-dash-empty">No spans captured for this request</div>';
    } else {
      var depthMap = {};
  ...
  var fetchCache = function () {
    fetchJSON(API + "/cache")
      ...
      .catch(function () {
        setInner(
          "ss-dash-cache-body",
          '<div class="ss-dash-empty">Cache not available</div>',
        );
      });
  };
  ...
  var renderCache = function (data) {
    ...
    if (keys.length === 0) {
      setInner(
        "ss-dash-cache-body",
        '<div class="ss-dash-empty">No cache keys found</div>',
      );
      return;
    }
  ...
  var fetchJobs = function () {
    ...
    fetchJSON(API + "/jobs?" + params)
      ...
      .catch(function () {
        setInner(
          "ss-dash-jobs-body",
          '<div class="ss-dash-empty">Jobs/Queue not available</div>',
        );
      });
  };
  ...
  var renderJobs = function (data) {
    ...
    var stats = data.stats || data.overview || {};
    var ps = getPage("jobs");
    ps.total = data.meta ? data.meta.total : data.total || items.length;
    ...
    if (items.length === 0) {
      setInner(
        "ss-dash-jobs-body",
        '<div class="ss-dash-empty">No jobs found</div>',
      );
    ...
    items.forEach(function (j) {
      ...
      html +=
        ...
        "<td>" +
        (j.status === "failed"
          ? '<button class="ss-dash-retry-btn" data-retry-id="' +
            j.id +
            '">Retry</button>'
          : "") +
        "</td>" +
  ...
  var bindRedactedButtons = function (container, prefix) {
    container
      .querySelectorAll("." + prefix + "-redacted-reveal")
      .forEach(function (btn) {
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
    ...
    container
      .querySelectorAll("." + prefix + "-redacted-copy")
      .forEach(function (btn) {
        btn.addEventListener("click", function (e) {
          e.stopPropagation();
          var wrap = btn.closest("." + prefix + "-redacted-wrap");
  ...
  var fetchConfig = function () {
    fetchJSON(API + "/config")
      ...
      .catch(function () {
        setInner(
          "ss-dash-config-body",
          '<div class="ss-dash-empty">Config not available</div>',
        );
      });
  };
  ...
  var renderConfig = function () {
    ...
    var source =
      configActiveTab === "env"
        ? configRawData.env || {}
        : configRawData.config || configRawData;
    ...
    if (term) {
      filtered = flat.filter(function (item) {
        var valStr = isRedactedObj(item.value)
          ? item.value.display
          : String(item.value);
        return (
          item.path.toLowerCase().indexOf(term) !== -1 ||
          valStr.toLowerCase().indexOf(term) !== -1
        );
      });
    }
        ...
        topKeys.forEach(function (sectionKey) {
          ...
          if (isObj) {
            html +=
              ...
              "</div>";
            html +=
              '<div class="ss-dash-config-section-body" style="display:none">';
  ...
  var formatConfigValue = function (val) {
    if (val === null || val === undefined)
      return '<span style="color:var(--ss-dim)">null</span>';
    if (val === true)
      return '<span style="color:var(--ss-green-fg)">true</span>';
    if (val === false)
      return '<span style="color:var(--ss-red-fg)">false</span>';
    if (typeof val === "number")
      return '<span style="color:var(--ss-amber-fg)">' + val + "</span>";
    if (Array.isArray(val)) {
      ...
      return (
        '<span style="color:var(--ss-purple-fg)">[' +
        esc(items.join(", ")) +
        "]</span>"
      );
    }
  ...
  if (expandAllBtn) {
    expandAllBtn.addEventListener("click", function () {
      ...
      if (!body) return;
      body
        .querySelectorAll(".ss-dash-config-section-body")
        .forEach(function (el) {
          el.style.display = "";
        });
  ...
  var fetchCustomPane = function (pane) {
    fetchJSON(pane.endpoint)
      .then(function (data) {
        ...
        var rows =
          getNestedValue(data, key) || (Array.isArray(data) ? data : []);
      ...
      .catch(function () {
        setInner(
          ...
          '<div class="ss-dash-empty">Failed to load ' +
            esc(pane.label) +
            "</div>",
        );
      });
  ...
  var renderPagination = function (section, ps) {
    ...
    var html =
      ...
      ">&laquo; Prev</button>";
    html +=
      '<span class="ss-dash-page-info">Page ' +
      ps.page +
      " of " +
      totalPages +
      "</span>";
  ...
  var updateBadge = function (section, count) {
    var badge = root.querySelector(
      '[data-ss-section="' + section + '"] .ss-dash-nav-badge',
    );
  ...
  var initTransmit = function () {
    ...
    if (
      typeof Transmit === "undefined" &&
      typeof window.Transmit === "undefined"
    ) {
      ssWarn(
    ...
    try {
      var TransmitClass =
        typeof Transmit !== "undefined" ? Transmit : window.Transmit;
      ...
      var transmit = new TransmitClass({
        ...
        onSubscription: function (channel) {
          ssLog(
            "Subscription active on channel: " +
              channel +
      ...
      transmitSub.onMessage(function (message) {
        try {
          ...
          var kind =
            event.type ||
            (event.avgResponseTime !== undefined ? "overview" : "unknown");
  ...
  var initRoute = function () {
    ...
    navItems.forEach(function (item) {
      item.classList.toggle(
        "ss-dash-active",
        item.getAttribute("data-ss-section") === section,
      );
    });
    root.querySelectorAll(".ss-dash-pane").forEach(function (p) {
