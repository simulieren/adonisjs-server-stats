...
(function () {
  ...
  if (themeBtn) {
    themeBtn.addEventListener("click", function () {
      ...
      if (typeof window.__ssApplyBarTheme === "function")
        window.__ssApplyBarTheme();
    });
  }
  ...
  const deepLink = (section, id) => {
    ...
    const href =
      dashboardPath + "#" + section + (id != null ? "?id=" + id : "");
    return (
      ' <a href="' +
      esc(href) +
  ...
  const formatCell = (value, col) => {
    if (value === null || value === undefined)
      return '<span class="ss-dbg-c-dim">-</span>';
      ...
      case "json": {
        ...
        const preview =
          typeof value === "object"
            ...
            : String(value);
        return (
          '<span class="ss-dbg-data-preview" style="cursor:default">' +
          esc(preview) +
          "</span>"
        );
      }
  ...
  tabs.forEach((tab) => {
    tab.addEventListener("click", () => {
      ...
      panel
        .querySelectorAll(".ss-dbg-pane")
        .forEach((p) => p.classList.remove("ss-dbg-active"));
    ...
    fetchJSON(BASE + "/queries")
      ...
      .catch(() => {
        queryBodyEl.innerHTML =
          '<div class="ss-dbg-empty">Failed to load queries</div>';
      });
  };

  const renderQueries = () => {
    const filter = (
      querySearchInput ? querySearchInput.value : ""
    ).toLowerCase();
    ...
    for (let j = 0; j < filtered.length; j++) {
      ...
      html +=
        ...
        "</span>" +
        (dupCount > 1
          ? ' <span class="ss-dbg-dup">x' + dupCount + "</span>"
          : "") +
        "</td>" +
  ...
  if (querySearchInput)
    querySearchInput.addEventListener("input", renderQueries);
  if (queryClearBtn) {
    ...
    fetchJSON(BASE + "/events")
      ...
      .catch(() => {
        eventBodyEl.innerHTML =
          '<div class="ss-dbg-empty">Failed to load events</div>';
      });
  };

  const renderEvents = () => {
    const filter = (
      eventSearchInput ? eventSearchInput.value : ""
    ).toLowerCase();
    ...
    eventBodyEl.querySelectorAll(".ss-dbg-data-full").forEach((el) => {
      el.addEventListener("click", () => {
        ...
        const preview = eventBodyEl.querySelector(
          '[data-ev-idx="' + idx + '"]',
        );
    ...
    fetchJSON(BASE + "/routes")
      ...
      .catch(() => {
        routeBodyEl.innerHTML =
          '<div class="ss-dbg-empty">Failed to load routes</div>';
      });
  };

  const renderRoutes = () => {
    const filter = (
      routeSearchInput ? routeSearchInput.value : ""
    ).toLowerCase();
    ...
    for (let i = 0; i < filtered.length; i++) {
      ...
      const isCurrent =
        ...
        currentPath.match(
          new RegExp("^" + r.pattern.replace(/:[^/]+/g, "[^/]+") + "$"),
        );
      html +=
        "<tr" +
  ...
  const fetchLogs = () => {
    fetchJSON(LOGS_ENDPOINT)
      ...
      .catch(() => {
        logBodyEl.innerHTML =
          '<div class="ss-dbg-empty">No log endpoint available</div>';
      });
  };
  ...
  const renderLogs = () => {
    ...
    if (entries.length === 0) {
      ...
      logBodyEl.innerHTML =
        '<div class="ss-dbg-empty">No log entries' + hint + "</div>";
      return;
    }
  ...
  const emailPreviewClose = document.getElementById(
    "ss-dbg-email-preview-close",
  );
      ...
      .catch(() => {
        if (emailBodyEl)
          emailBodyEl.innerHTML =
            '<div class="ss-dbg-empty">Failed to load emails</div>';
      });
  };
  ...
  const renderEmails = () => {
    ...
    const filter = (
      emailSearchInput ? emailSearchInput.value : ""
    ).toLowerCase();
  ...
  if (emailSearchInput)
    emailSearchInput.addEventListener("input", renderEmails);
  if (emailClearBtn) {
  ...
  const fetchTraces = () => {
    ...
    fetchJSON(BASE + "/traces")
      ...
      .catch(() => {
        if (tlBodyEl)
          tlBodyEl.innerHTML =
            '<div class="ss-dbg-empty">Failed to load traces</div>';
      });
  };
  ...
  const fetchTraceDetail = (id) => {
    fetchJSON(BASE + "/traces/" + id)
      ...
      .catch(() => {
        if (tlWaterfall)
          tlWaterfall.innerHTML =
            '<div class="ss-dbg-empty">Failed to load trace</div>';
      });
  };
  ...
  const showTimeline = (trace) => {
    ...
    if (spans.length === 0) {
      html +=
        '<div class="ss-dbg-empty">No spans captured for this request</div>';
    } else {
    ...
    if (trace.warnings && trace.warnings.length > 0) {
      ...
      for (let w = 0; w < trace.warnings.length; w++) {
        html +=
          '<div class="ss-dbg-tl-warning">' + esc(trace.warnings[w]) + "</div>";
      }
      html += "</div>";
    }
      ...
      .catch(() => {
        if (cacheBodyEl)
          cacheBodyEl.innerHTML =
            '<div class="ss-dbg-empty">Cache not available</div>';
  ...
  const renderCache = () => {
    ...
    const filter = (
      cacheSearchInput ? cacheSearchInput.value : ""
    ).toLowerCase();
    ...
    if (cacheSummaryEl) {
      cacheSummaryEl.textContent =
        (stats.keyCount || keys.length || 0) + " keys";
    }

    let filtered = keys;
    if (filter) {
      filtered = keys.filter(
        (k) => (k.key || "").toLowerCase().indexOf(filter) !== -1,
      );
    }
      ...
      .catch(() => {
        if (jobsBodyEl)
          jobsBodyEl.innerHTML =
            '<div class="ss-dbg-empty">Jobs/Queue not available</div>';
  ...
  const renderJobs = () => {
    ...
    for (let i = 0; i < items.length; i++) {
      ...
      html +=
        ...
        "<td>" +
        (j.status === "failed"
          ? '<button class="ss-dbg-retry-btn" data-retry-id="' +
            j.id +
            '">Retry</button>'
          : "") +
        "</td>" +
  ...
  const formatConfigValue = (val) => {
    if (val === null || val === undefined)
      return '<span class="ss-dbg-config-val-null">null</span>';
    if (val === true) return '<span class="ss-dbg-config-val-true">true</span>';
    if (val === false)
      return '<span class="ss-dbg-config-val-false">false</span>';
    if (typeof val === "number")
      return '<span class="ss-dbg-config-val-number">' + val + "</span>";
    if (Array.isArray(val)) {
      ...
      return (
        '<span class="ss-dbg-config-val-array">[' +
        esc(items.join(", ")) +
        "]</span>"
      );
    }
    if (typeof val === "object") {
      try {
        return (
          '<span class="ss-dbg-config-val-null">' +
  ...
  const isRedactedObj = (val) =>
    val && typeof val === "object" && val.__redacted === true;
    ...
    container
      ...
      .forEach((btn) => {
        btn.addEventListener("click", (e) => {
          ...
          if (!wrap) return;
          const display = wrap.querySelector(
            "." + prefix + "-redacted-display",
          );
      ...
      .catch(() => {
        if (configBodyEl)
          configBodyEl.innerHTML =
            '<div class="ss-dbg-empty">Config not available</div>';
      });
  };
        ...
        for (let t = 0; t < topKeys.length; t++) {
          ...
          if (isObj) {
            html +=
              ...
              "</div>";
            html +=
              '<div class="ss-dbg-config-section-body" style="display:none">';
  ...
  const fetchCustomPane = (pane) => {
    ...
    fetchJSON(pane.endpoint)
      ...
      .catch(() => {
        if (bodyEl)
          bodyEl.innerHTML =
            '<div class="ss-dbg-empty">Failed to load ' +
            esc(pane.label) +
            "</div>";
      });
  };
  ...
  const initTransmit = () => {
    ...
    const TransmitClass =
      typeof window !== "undefined" && window.Transmit ? window.Transmit : null;
  ...
  const updateBarQueryBadge = () => {
    ...
    fetchJSON(BASE + "/queries")
      .then((data) => {
        ...
        if (valEl) {
          valEl.textContent =
            (s.total || 0) + " / " + (s.avgDuration || 0).toFixed(1) + "ms";
          valEl.className =
            "ss-value " + (s.slow > 0 ? "ss-amber" : "ss-green");
        }
      })
      .catch(() => {});
  };
