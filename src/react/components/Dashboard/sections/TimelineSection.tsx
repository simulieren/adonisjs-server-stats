    ...
    return (
      <div>
        <div className="ss-dash-detail-header">
          <button type="button" className="ss-dash-btn" onClick={handleBack}>Back</button>
        ...
        {trace.warnings && trace.warnings.length > 0 && (
          <div className="ss-dash-warnings">
            ...
            {trace.warnings.map((w: string, i: number) => (
              <div key={i} className="ss-dash-warning">{w}</div>
            ))}
          </div>
        )}
      </div>
        ...
        <>
          <DataTable
            columns={[
              ...
              { key: 'url', label: 'URL', render: (v: string) => <span style={{ color: 'var(--ss-text)' }}>{v}</span> },
              { key: 'status_code', label: 'Status', width: '60px', render: (v: number) => <StatusBadge code={v} /> },
              { key: 'total_duration', label: 'Duration', width: '80px', render: (v: number) => formatDuration(v) },
