...
function isRedactedObj(val: ConfigValue): val is RedactedValue {
  return (
    val !== null &&
    typeof val === "object" &&
    !Array.isArray(val) &&
...
function flattenConfig(obj: ConfigValue, prefix: string): FlatEntry[] {
  if (typeof obj !== "object" || obj === null || obj === undefined) {
    return [{ path: prefix, value: obj }];
  }
  if (Array.isArray(obj) || isRedactedObj(obj)) {
    return [{ path: prefix, value: obj }];
  }
...
function formatFlatValue(value: ConfigValue): { text: string; color?: string } {
...
function countLeaves(obj: ConfigValue): number {
  if (
    obj === null ||
    obj === undefined ||
    typeof obj !== "object" ||
...
function collectTopLevelObjectKeys(obj: ConfigValue): string[] {
  if (
    obj === null ||
    obj === undefined ||
    typeof obj !== "object" ||
...
function copyWithFeedback(
  ...
  navigator.clipboard
    ...
    .then(() => {
      ...
      btnRef.textContent = "\u2713";
      btnRef.classList.add(`${p}-copy-row-ok`);
      setTimeout(() => {
        btnRef.textContent = prev;
        btnRef.classList.remove(`${p}-copy-row-ok`);
      }, 1200);
    })
    .catch(() => {
      // Silently fail
            ...
            return (
              <tr key={key}>
                ...
                <td className={`${p}-env-val`}>
                  {redacted ? (
                    <RedactedToggle
                      redacted={value as RedactedValue}
                      p={p}
                    />
                  ) : (
            ...
            return (
              <tr key={item.path}>
                ...
                <td>
                  {redacted ? (
                    <RedactedToggle
                      redacted={item.value as RedactedValue}
                      p={p}
                    />
                  ) : (
                    <span
                  ...
                  {!redacted && (
                    <button
                      ...
                      onClick={(e) => {
                        ...
                        copyWithFeedback(
                          copyVal,
                          btnRefs.current.get(item.path) ?? null,
                          p,
                        );
                      }}
                    >
      ...
      {keys.map((key) => {
        ...
        return (
          <div key={key} className={`${p}-config-section`}>
            ...
            >
              {isObject ? (
                <span className={`${p}-config-toggle`}>
                  {isExpanded ? "\u25BC" : "\u25B6"}
                </span>
              ) : (
                <span
                  className={`${p}-config-toggle`}
              ...
              <span className={`${p}-config-key`}>{key}</span>
              {isObject ? (
                <span className={`${p}-config-count`}>
                  {countLeaves(value)} entries
                </span>
              ) : (
                <>
                  <span
